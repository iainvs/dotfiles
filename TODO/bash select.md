...
function hideCursor {
  # escape code for hiding the cursor
  printf '\033[?25l' 
  
  # capture CTRL+C to show cursor, write a new line, then exit the script 
  trap 'showCursor; echo; exit 0' SIGINT 
}

function showCursor {
  # escape code for showing the cursor
  printf '\033[?25h'
  # removes the trap for CTRL+C
  trap - SIGINT
}

function clearLastMenu {
  # stores the number of lines of var menuStr to msgLineCount
...
function renderMenu {
  ...
  # Get the longest item from the list so that we know how many spaces to add to avoid overlaps
...
function getChoice {
  ...
  local maxViewable=0
  local instruction="Choose a connection protocol."
